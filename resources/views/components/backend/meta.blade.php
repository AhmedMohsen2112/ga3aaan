...
<link href="{{url('public/backend/scripts')}}/jspdf.debug.js" rel="stylesheet" type="text/css" />
@if ($lang_code == 'ar')
<style>
    .datetimepicker {
        float: right
...
<script>
    var config = {
        ...
        lang_code: "{{$lang_code}}",
        pusher_app_id: '98a7171327a738360056',
        pusher_cluster: 'eu',
        pusher_encrypted: true,
        resturant: "{{$User->resturant?$User->resturant->id:false}}",
    };
    var lang = {
        choose_at_least_one: "{{ _lang('app.choose_at_least_one')}}",
        add: "{{ _lang('app.add')}}",
        edit: "{{ _lang('app.edit')}}",
        save: "{{ _lang('app.save')}}",

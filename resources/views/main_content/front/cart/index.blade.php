...
<div class="container">
    <div class="centerbolog">
        <h2 class="title hidden-xs">{{ $page_title }}</h2>
        ...
        <div class="col-md-12" id="cart-content">
                    ...
                    <li class="basket-det">
                        ...
                        <div class="row item-row">
                            <div class="top col-sm-10 size">
                                ...
                                <p>{{isset($one['size_id'])?$one[$title_slug].' ( '.$one['size_'.$title_slug].' )':$one[$title_slug]}}</p>
                                <p class="pull-left" id="{{'total_price_'.$key}}">{{$one['total_price'].' '.$currency_sign}}</p>

                            </div>
                            @if(count($one['sub_choices']))
                            <div class="bottom col-sm-10">
                            <h2>{{_lang('app.choices').' :'}}</h2>
                            </div>
                            @foreach($one['sub_choices'] as $sub_choice)
                            <div class="bottom col-sm-10">
                                <p style="font-size:14px; color:#656565;">{{$sub_choice[$title_slug]}}</p>
                            </div>
                            @endforeach
                            @endif
            ...
            <div class="orderbasc ">
                ...
                <div class="total">
                    <p>المجموع</p>
                    <p id="total_price" class="{{isset($cart['price_list']['net_price'])?'old-price':''}}">{{$cart['price_list']['total_price'].' '.$currency_sign}} </p>
                    <p id="net_price" style="display: {{isset($cart['price_list']['net_price'])?'block':'none'}};">{{isset($cart['price_list']['net_price'])?$cart['price_list']['net_price']:''}} </p>
                </div>

            ...
            <div class="portlet-body">
                <div class="tabbable">
                    <ul class="nav nav-tabs nav-tabs-lg">
                        ...
                        @if($User->type==2||($User->type==1&&$order->status==0))
                        <li class="">
                                ...
                                <div class="col-md-6">
                                    <div class="well">
                                        ...
                                        <div class="row static-info align-reverse">
                                            ...
                                            <div class="col-md-3 value">
                                                {{( $order->net_cost*$order->commission)/100 }} {{ $currency_sign }}
                                            </div>
                                        </div>
                                        @else
                                        <div class="row static-info align-reverse">
                            ...
                            <div class="row">
                                <form action="{{ route('order_status') }}" method="post" id="orderStatusForm">
                                    ...
                                    <div class="form-group form-md-line-input col-md-3">
                                        @if ($order->status == 1 && $order->canEdit)
                                        <p>{{ _lang('app.please_,_waiting').' '.' 3 '._lang('app.minutes') }}</p>
                                        @elseif ($order->status == 5)
                                        <p>{{ _lang('app.order_is_under_modification') }}</p>
                                        @elseif ($order->status == 6)
                                        <p>{{ _lang('app.order_is_cancelled') }}</p>
                                        @elseif ($order->status == 4)
                                        <p>{{ _lang('app.rejected').' : '.$order->refusing_reason }}</p>
                                        @else
                                        @if ($order->status == 7)
                                        <p>{{ _lang('app.client_finish_modification') }}</p>
                                        @endif
                                    ...
                                    <div class="form-group form-md-line-input col-md-6" id="refusing" style="display: none;">
                                        <textarea rows="5" class="form-control" id="refusing_reason" name="refusing_reason"></textarea>

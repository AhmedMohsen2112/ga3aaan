...
var Cart = function () {
    ...
    var init = function () {
        ...
        handleChangeLocationSubmit();

    }
    var getChoices = function (meal_id, meal_size_id) {
        var url = config.url + "/meal-choices/";
        var data = {meal_id: meal_id};
        if (meal_size_id) {
            data.meal_size_id = meal_size_id
        }
        // setTimeout(function () {
        $.ajax({
            url: url,
                    ...
                    success: function (data)
                    {
                        ...
                        if (data.type == 'success') {
                            if (data.data.items.length == 0) {
                                ...
                                $('#total_price').addClass('old-price');
                                $('#net_price').html(PriceList.net_price + ' ' + lang.currency_sign).show();
                                $('#coupon')
                    ...
                    success: function (data)
                    {
                        ...
                        if (data.type == 'success') {
                            if (data.data.items.length == 0) {
                                ...
                                $('#total_price_' + index).html(items[index].total_price + ' ' + lang.currency_sign)
...
//            var index = $('.remove-cart').index(this);
    ...
    var handleChangeLocationSubmit = function () {
...
//                },
        ...
        $('#changeLocationForm .submit-form').click(function () {
            var validate_2 = $('#changeLocationForm').validate().form();
            if (validate_2) {
                $('#changeLocationForm .submit-form').prop('disabled', true);
                $('#changeLocationForm .submit-form').html('<i class="fa fa-spinner fa-spin fa-2x fa-fw"></i><span class="sr-only">Loading...</span>');
                setTimeout(function () {
        ...
        $('#changeLocationForm input').keypress(function (e) {
            if (e.which == 13) {
                var validate_2 = $('#changeLocationForm').validate().form();
                if (validate_2) {
        ...
        $('#changeLocationForm').submit(function () {
            var url = config.url + "/change-location";
            var formData = new FormData($(this)[0]);
            formData.append('resturant', _config.resturant_id)
            formData.append('meal', _config.meal_id)
            formData.append('menu_section', _config.menu_section_id)
            $.ajax({
                url: url,
                type: 'POST',
                dataType: 'json',
                ...
                {
                    console.log(data);
                    $('#changeLocationForm .submit-form').prop('disabled', false);
                    $('#changeLocationForm .submit-form').html(lang.save);
                    ...
                    if (data.type == 'success') {
                        ...
                        if (typeof data.errors !== 'undefined') {
                            for (i in data.errors)
                            {
                                $('[name="' + i + '"]')
                                        .closest('.form-group').addClass('has-error').removeClass("has-info");
                                $('[name="' + i + '"]').closest('.form-group').find(".help-block").html(data.errors[i])
                            }
                        }
    ...
    var handleAddToCart = function () {
        ...
        $('#addToCartForm').submit(function () {
            ...
            formData.append('resturant_slug', _config.resturant_slug);
             formData.append('quantity',  $('input[name="qty').val());
            if ($('input[name="size"]').length > 0) {
                formData.append('size_id', $('input[name="size"]:checked').val());
            }
                ...
                success: function (data)
                {
                    ...
                    if (data.type == 'success') {
                        ...
                        if (typeof data.errors !== 'undefined') {
                            ...
                            {
                                $('#' + i).find('p').remove();
                                $('#' + i).find('.help-message').html('<i class="fa fa-exclamation-triangle"></i> '+data.errors[i]);
                                $('#' + i).find('.help-message').show();
                            }
                ...
                success: function (data)
                {
                    ...
                    if (data.type == 'success') {
                        ...
                        $('#newMessageModal').on('hidden.bs.modal', function () {
                            window.location.href = config.current_orders;
                        })
...
//        $('#addToCartModal').modal('show');
            if (_config.resturant_branch_id && _config.is_open && _config.in_same_branch) {
                $('#addToCartModal').modal('show');
            } else if (!_config.is_open) {
                $('#newMessageModal .message-content').html(lang.sorry_resturant_is_closed);
                $('#newMessageModal').modal('show');
            } else if (!_config.in_same_branch) {
    ...
    return {
        ...
        addToCart: function (el) {
            ...
            var meal_size_id = false;
            if (_config.has_sizes == 1) {
                meal_size_id = $('input[name="size"]:checked').val();

            }
            getChoices(_config.meal_id, meal_size_id);
        ...
        emptyCart: function (el) {
            ...
            $(el).html('<i class="fa fa-spinner fa-spin fa-2x fa-fw"></i><span class="sr-only">Loading...</span>');
            var url = config.url + "/cart/empty";
            $.ajax({
                url: url,
                type: 'GET',
                dataType: 'json',

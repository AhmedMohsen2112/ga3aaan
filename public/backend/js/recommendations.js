...
var Recommendations = function () {
    ...
    var handleRecords = function () {
        Recommendations_grid = $('.dataTable').dataTable({
            //"processing": true,
            "serverSide": true,
            "ajax": {
                "url": config.admin_url + "/recommendations/data",
                "type": "POST",
                "data": {_token: $('input[name="_token"]').val()},
            },
            "columns": [
                {"data": "input", orderable: false},
                {"data": "resturant_name","name":"recommendations.resturant_name"},
                {"data": "region","name":"recommendations.region"},
                {"data": "user"},
                {"data": "email","name":"users.email"},
                {"data": "created_at","name":"recommendations.created_at"},
            ],
            "order": [
                [4, "desc"]
            ],
    ...
    return {
        ...
        delete: function (t) {
            ...
            if (ids.length > 0) {
                ...
                setTimeout(function () {
                    ...
                    My.deleteForm({
                        element: t,
                        url: config.admin_url + '/recommendations/delete',
                        data: {_method: 'DELETE',ids: ids,_token: $('input[name="_token"]').val()},
                        success: function (data)
                        {
                            $(t).prop('disabled', false);
                            $(t).html(lang.delete);
                            Recommendations_grid.api().ajax.reload();
                            enableOrDisableDeleteBtn();
                            ids = [];
                        }
                    });
        ...
        viewMessage: function (ele) {
            ...
            setTimeout(function () {
                $.ajax({
                    url: config.admin_url + "/Recommendations/"+$(ele).data('id'),
                    async: false,
                    success: function (data) {
                        console.log(data);

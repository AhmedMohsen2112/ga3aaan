    ...
    public function show($id) {
        ...
        try {
            ...
            $order = Order::withTrashed()->join('addresses', 'orders.user_address_id', '=', 'addresses.id')
                    ->select('orders.id as order_id', 'orders.net_cost', 'orders.acceptance_date as acceptance_date', 'orders.vat', 'orders.primary_price', 'orders.total_cost', 'orders.toppings_price', 'orders.coupon', 'orders.service_charge', 'orders.delivery_cost', 'orders.resturant_id', 'orders.status', 'orders.is_rated', 'addresses.*')
                    ->where('orders.id', $order_id)
            ...
            if ($order_meals->count() > 0) {
                foreach ($order_meals as $meal) {
...
//            dd($order_meals);
    ...
    public function edit($id) {
        ...
        foreach ($order_meals as $order_meal) {
            $meal = array(
                ...
                'title_ar' => $order_meal->meal_title_ar,
                'title_en' => $order_meal->meal_title_en,
                'price' => $order_meal->cost_of_meal,
                'quantity' => $order_meal->quantity,
                'primary_price' => $order_meal->cost_of_meal + $order_meal->sub_choices_price,
                'sub_choices_price' => $order_meal->sub_choices_price,
    ...
    private function getOrderMealChoices($order_meal_id) {
        $choices = OrderMeal::join('order_meal_choices', 'order_meals.id', '=', 'order_meal_choices.order_meal_id')
                ...
                ->select([
                    "sub_choices.title_ar", "sub_choices.title_en", "order_meal_choices.price",
                ])
                ->get();
